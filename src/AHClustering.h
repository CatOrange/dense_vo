...
#pragma omp parallel for
	...
	int planeExtraction(ushort* depth, int height, int width, double fx, double fy, double cx, double cy)
	{
		...
		for (int i = 0; i < n; i++){
		for (int j = 0; j < m; j++){
		showImage2.at<uchar>(i, j) = nodeMap[i*m+j].valid*255 ;
		}
		}
		...
		*/
		...
		{
			int parent = mySet.find(i);

			if (nodeMap[parent].num > DISPLAY_NUM){
				nodeMap[parent].num = 0;
				MQ.insert(std::map<int, int>::value_type(parent, sum));
				sum++;
				//numInPlane[currentParent] = nodeMap[currentParent].num;
				//	puts("DEBUG: found plane");
			}

			/*	if ( validPlane[parent] == false){
			continue;
			}*/
			//if (countVst[parent] == false){
			//	countVst[parent] = true;
			//	sum++;
			//}
		}
		...
		Mat showImage(height, width, CV_8UC3);
		for (int i = 0; i < height; i += winH)
		{
			for (int j = 0; j < width; j += winW)
			{
				int ii = i / winH;
				int jj = j / winW;
				int parent = mySet.find(nodeMapID[ii][jj]);
				if (nodeMap[parent].valid == false){
					continue;
				}
				iter = MQ.find(parent);
				if (iter == MQ.end()){
				...
				*/	
				...
				for (int u = uBegin; u < uEnd; u++)
				{
					for (int v = vBegin; v < vEnd; v++)
					{
						showImage.at<cv::Vec3b>(u, v)[0] = R[iter->second];
						showImage.at<cv::Vec3b>(u, v)[1] = G[iter->second];
						showImage.at<cv::Vec3b>(u, v)[2] = B[iter->second];
					}
				}
			}
		}
		imshow("debug", showImage);
